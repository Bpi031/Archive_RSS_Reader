...
app.get('/rss/:id', async (req, res) => {
  ...
  try {
    ...
    for (const item of feed.items) {
      htmlContent += `
        <li class="p-4 bg-gray-50 rounded-lg shadow-md flex items-center relative">
          <img src="${getFaviconUrl(item.link)}" alt="Favicon" class="w-4 h-4 mr-2">
          <a href="#" onclick="archiveUrl('${item.link}', this)" class="text-lg font-semibold text-indigo-600 hover:underline">${item.title}</a>
...
app.post('/archive', async (req, res) => {
  const { url } = req.body;
  console.log('URL received for archiving:', url);
